...
H~0~: **pH does not predict abalone growth (chnage in shell surface area per day mm^2^**  

H~A~: **pH does not predict abalone growth (chnage in shell surface area per day mm^2^G**
...
```
**Residuals seem homeoscedastic (consistent across values) based on Residuals vs Fitted and Scale-Location plots. Residuals seem normally distributed based on the QQ plot, the points mostly follow the reference line. There were also no outliers that might influence model estimates or predictions.  **
...
**With each increase in pH, the model predicts  an increase of 3.0 +- in change in shell surface area  per day (mm2 d-1))**
What is the slope estimate (with standard error?)  

**When pH = 0, the model preicts that abalone change in shell surface area per day is -18.5. (Note to self: this is not necessarily biological realistic, but is something the model generates**  
...
**x is predictior values for pH**

**predicted is the model predictions for change area per day**
**st error: (Standar error describing precision in model predictions**
**Conf. low and conf. high (lower and uper** 
))
...
**At pH = 8 the model predicts that abalone change in shell area per day id... **
